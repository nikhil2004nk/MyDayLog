...
export default function Calendar({ year, month, tiffinByDate, onSelect, onPrevMonth, onNextMonth, onToday, today, selected, selectedDates, weekStart = 'mon' }: CalendarProps) {
            ...
            {w.map((d, j) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Desktop: inline row [lunch] [date] [dinner] */}
                  <div className="hidden md:flex items-center justify-center gap-1 pointer-events-none">
                    {tiff?.lunch && (
                      <span
                        title={tiff.lunch === 'received' ? 'Lunch received' : 'Lunch skipped'}
                        className={`${tiff.lunch === 'received' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300'} inline-flex items-center justify-center rounded-full px-1.5 py-0.5 text-sm leading-none`}
                    ...
                    {tiff?.dinner && (
                      <span
                        ...
                        className={`${tiff.dinner === 'received' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300'} inline-flex items-center justify-center rounded-full px-1.5 py-0.5 text-sm leading-none`}
